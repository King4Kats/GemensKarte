...
def construire_requete() -> str:
    ...
    return "SELECT " + ",\n       ".join(morceaux) + " FROM associations"


def main():
    with psycopg.connect(DSN) as c:
        cur = c.execute(construire_requete())  # execute() renvoie le curseur en psycopg3
        row = cur.fetchone()
    ...
    total = d["total"]
    print(f"\nSuivi des passes de découverte ciblées — {total} assos au total (Vendée)\n")
