// Départements couverts par les pages SEO (toute la France + DROM depuis l'import
// national). Pour chaque code INSEE : nom + slug d'URL + région. On n'utilise pas de
// forme grammaticale "de la / du / des" (ingérable proprement sur 101 départements) :
// les pages SEO emploient une formulation neutre ("Associations par commune — <Nom>").
...
// [code, nom, région] pour tous les départements (métropole + Corse + DROM).
...
export const SEO_DEPARTEMENTS: Record<string, { nom: string; slug: string; region: string }> =
  Object.fromEntries(DEPTS.map(([code, nom, region]) => [code, { nom, slug: slugify(nom), region }]));
